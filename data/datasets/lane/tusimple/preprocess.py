...
DATASETS_TRAIN = {
    'train1': 'train_set/label_data_0313.json',
    'train2': 'train_set/label_data_0531.json',
    'val': 'label_data_0601.json',  # train_set 밖
}

DATASETS_VAL = {
    'test': 'test_label.json',      # train_set 밖
}
...
def lane_data_prep(root_path,
    ...
    for key, fpath in DATASETS_TRAIN.items():
        ...
        with open(file_path, "r") as f:
            train_data_samples = f.readlines()
...
class INfoProcessor(object):
    ...
    def obtain_img_info(self, sample):
        ...
        img_name = data['raw_file'].replace('/', ',')

        img = cv2.imread(img_path)
        if img is None:
            print(f"[Warning] {img_path} not found or unreadable. Skipping this sample.")
            return None
    ...
    def __call__(self):
        if self.num_workers > 1:
            ...
            for data_infos_ in data_infos:
                data_infos_all.extend([d for d in data_infos_ if d is not None])
        else:
            data_infos_all = [d for d in self._process_single(0) if d is not None]
        return data_infos_all

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Data preprocessing for nuScenes dataset.')
    parser.add_argument('--root', type=str, default='/data/sets/tusimple')
