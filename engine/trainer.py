...
class Trainer(object):
    ...
    def __call__(self, data_loader, epoch, best_prec1):
        ...
        for data_dict in data_loader:
                ...
                for v in loss_dict.values():
                    if torch.is_tensor(v):
                        if getattr(v, 'is_sparse', False):
                            loss_values.append(v.to_dense().float().sum())
                        else:
                            loss_values.append(v.float().sum())
                    else:
                        loss_values.append(float(v))
                loss = sum(loss_values) / self.accum_steps
            ...
            step += 1
            loss_val = loss.detach().cpu().numpy() * self.accum_steps
            loss_avg.update(loss_val, len(images))
            for k, v in loss_dict.items():
                try:
                    loss_dict_avg[k].update(v.detach().cpu().float().sum().item(), len(images))
                except:
                    # float가 아니면 그냥 0 처리
                    loss_dict_avg[k].update(0.0, len(images))
